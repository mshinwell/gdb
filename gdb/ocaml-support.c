...
static void *ocaml_support_init(gdb_ocaml_support_t *stubs)
{
  void *handle = NULL;
  ...
  {
    fprintf(stderr, "ocaml support: missing symbol gdb_ocaml_support_init in %s\n", LIB_SUPPORT);
    dlclose(handle);
    return NULL;
  }
...
gdb_ocaml_support_t *ocaml_support_library(void)
{
  ...
  static gdb_ocaml_support_t stubs;
...
int ocaml_support_val_print (struct type *type, const gdb_byte *valaddr,
                             ...
                             int depth)
{
  gdb_ocaml_support_t *stubs = ocaml_support_library();
  if (stubs && stubs->val_print)
  {
    stubs->val_print (type,valaddr,embedded_offset,address,stream,recurse,val,options,depth);
    return 1;
  }
  return 0;
}
